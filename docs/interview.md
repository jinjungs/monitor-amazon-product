...
이런 갭들의 공통점은 **코드 자체는 동작하지만, 실제로 사용해보거나 흐름을 따라가지 않으면 보이지 않는 문제**들이다.

**다음에 보완할 방법:** 테스트 시나리오를 미리 잘 작성하는 것. 단위 테스트뿐 아니라 "사용자가 상품을 등록하고 알림을 받기까지의 전체 흐름"처럼 end-to-end 시나리오를 먼저 정의해두면, AI가 생성한 코드의 갭이 훨씬 빨리 드러난다.
