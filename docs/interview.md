...
## Implementation vs Spec Gaps

**Q. @Retryable이 5xx에도 적용되지 않는 문제:**

`clarify-requirements.md`에 명확히 정의해뒀다:
...
하지만 실제 구현에서 404 버그를 수정할 때 `HttpStatusException`을 `noRetryFor`에 추가하면서 **4xx와 5xx를 구분하지 않고 둘 다 retry 제외**시켰다. `HttpStatusException`은 4xx/5xx를 모두 포함하는 클래스이기 때문이다.

요구사항에 적어뒀음에도 구현 시 놓친 케이스.
...
```java
    ...
    if (e.getStatusCode() >= 500) {
        throw new IOException("5xx: " + e.getStatusCode()); // retryFor에 해당 → retry
    }
    throw e; // 4xx → noRetryFor에 해당 → no retry
}
```
